...
class ImageCacheController extends BaseController
{
    ...
    public function getImage($template, $filename)
    {
        ...
        $manager = new ImageManager(Config::get('imagecache'));
    ...
    {
        $template = config("imagecache.templates.{$template}", $template);
        ...
        switch (true) {
            ...
            default:
                $match = preg_match("/([0-9]+|auto)x([0-9]+|auto)/i", $template, $vars);
                
                if (! $match) return abort(404);
                
                $width  = (integer) $vars[1] ?: null;
                $heigth = (integer) $vars[2] ?: null;

                return function($image) use ($width, $heigth) {
                    if ($width == null || $heigth == null) { 
                        return $image->resize($width, $heigth, function ($constraint) {
                            $constraint->aspectRatio();
                            $constraint->upsize();
                        });
                    }

                    return $image->fit($width, $heigth);
                };
